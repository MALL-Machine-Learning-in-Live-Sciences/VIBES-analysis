#SVA
sva.combat = function(dataset, par = TRUE, mo = FALSE, plt = FALSE){
  require(sva)
  target = dataset$target
  batch = dataset$Study
  meta = data.frame(study = dataset$Study,
                    target = dataset$target)
  rownames(meta) = rownames(dataset)
  design <- model.matrix(~1, data=meta)
  cols <- sapply(dataset, is.numeric)
  mat = as.matrix(dataset[cols])
  Combat = ComBat(dat=t(mat), batch=batch, ref.batch = "Ravel", mean.only=mo,
                  mod=design, par.prior=par, prior.plots=plt)
  mat = as.data.frame(t(Combat))
...
AD_21[1:21] = mat_AD21
class(mat_AD21) <- 'matrix'

#Apply sva
sva.data = sva.combat(AD_21)
show.pca(AD_21, batch = "Study")
show.pca(sva.data, batch = "batch")

#Cluster
Ravel.sva = subset(sva.data, batch == "Ravel")
Sriniv.sva = subset(sva.data, batch == "Srinivasan")
Ravel.sva_2C = make.cluster(dataset = Ravel.sva, k = 2)
Ravel.sva_3C = make.cluster(dataset = Ravel.sva, k = 3)
Sriniv.sva.predict_2C = make.cl_predict(lista = Ravel.sva_2C, newdata = Sriniv.sva)
Sriniv.sva.predict_3C = make.cl_predict(lista = Ravel.sva_3C, newdata = Sriniv.sva)

#Train
bmr_2C = ML.exec_C2(dataset = Ravel.sva_2C$dataframe)
bmr_3C = ML.exec_C3(dataset = Ravel.sva_3C$dataframe)

#Predict
require(caret)
bm2 =get.best.model_C2(bncmark = bmr_2C)
test_task2 = makeClassifTask(data = Sriniv.sva.predict_2C[1:22], target = "cluster")
predict2 = predict(bm2, task = test_task2, type = "prob")
CM2 = confusionMatrix(data = predict2$data$response, reference = as.factor(Sriniv.sva.predict_2C$cluster))

bm3 = get.best.model_C3(bncmark = bmr_3C)
test_task3 = makeClassifTask(data = Sriniv.sva.predict_3C[1:22], target = "cluster")
predict3 = predict(bm3, task = test_task3, type = "prob")
CM3 = confusionMatrix(data = predict3$data$response, reference = as.factor(Sriniv.sva.predict_3C$cluster))
