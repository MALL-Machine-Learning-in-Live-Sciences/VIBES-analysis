...
limma.combat = function(dataset){
  ...
  limma<- removeBatchEffect(t(mat), batch=batch, design=design)
  mat = as.data.frame(t(limma))
  dataframe = cbind(mat, target, batch)
  return(dataframe)
}
...
#Apply limma
limma.data = limma.combat(AD_21)
show.pca(AD_21, batch = "Study")
show.pca(limma.data, batch = "batch")

#Cluster
Ravel.limma = subset(limma.data, batch == "Ravel")
Sriniv.limma = subset(limma.data, batch == "Srinivasan")
Ravel.limma_2C = make.cluster(dataset = Ravel.limma, k = 2)
Ravel.limma_3C = make.cluster(dataset = Ravel.limma, k = 3)
Sriniv.limma.predict_2C = make.cl_predict(lista = Ravel.limma_2C, newdata = Sriniv.limma)
