...
PN.combat = function(dataset){
  ...
  target = AD_21$target
  batch = AD_21$Study
  # Sum 1 to avoid 0 values with log ratio transformation
  AD_21[1:21] = AD_21[1:21] +1
  mat_AD21 = logratio.transfo(AD_21[1:21], logratio = 'CLR')
  class(mat_AD21) <- 'matrix' 
  AD_21.tss <- t(apply(AD_21[1:21], 1, function(x){x/sum(x)}))
  PN <- percentile_norm(data = AD_21.tss, batch = as.factor(AD_21$Study), 
                        trt = as.factor(AD_21$target))
  
  dataframe = as.data.frame(cbind(PN, target, batch))
  return(dataframe)
}
...
show.pca(pn.data, batch = "batch")

#Cluster
Ravel.pn = subset(pn.data, batch == "Ravel")
Sriniv.pn = subset(pn.data, batch == "Srinivasan")
Ravel.pn_2C = make.cluster(dataset = Ravel.pn, k = 2)
Ravel.pn_3C = make.cluster(dataset = Ravel.pn, k = 3)
Sriniv.pn.predict_2C = make.cl_predict(lista = Ravel.pn_2C, newdata = Sriniv.pn)
Sriniv.pn.predict_3C = make.cl_predict(lista = Ravel.pn_3C, newdata = Sriniv.pn)

#Train
bmr_2C = ML.exec_C2(dataset = Ravel.pn_2C$dataframe)
bmr_3C = ML.exec_C3(dataset = Ravel.pn_3C$dataframe)

#Predict
require(caret)
bm2 =get.best.model_C2(bncmark = bmr_2C)
test_task2 = makeClassifTask(data = Sriniv.pn.predict_2C[1:22], target = "cluster")
predict2 = predict(bm2, task = test_task2, type = "prob")
CM2 = confusionMatrix(data = predict2$data$response, reference = as.factor(Sriniv.pn.predict_2C$cluster))

bm3 = get.best.model_C3(bncmark = bmr_3C)
test_task3 = makeClassifTask(data = Sriniv.pn.predict_3C[1:22], target = "cluster")
predict3 = predict(bm3, task = test_task3, type = "prob")
CM3 = confusionMatrix(data = predict3$data$response, reference = as.factor(Sriniv.pn.predict_3C$cluster))
