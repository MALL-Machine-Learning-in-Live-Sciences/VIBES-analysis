...
#Cluster
Ravel.mmu = subset(mmu.data, batch == "Ravel")
Sriniv.mmu = subset(mmu.data, batch == "Srinivasan")
Ravel.mmu_2C = make.cluster(dataset = Ravel.mmu, k = 2)
...
test_task2 = makeClassifTask(data = Sriniv.mmu.predict_2C[1:22], target = "cluster")
predict2 = predict(bm2, task = test_task2, type = "prob")
CM2 = confusionMatrix(data = predict2$data$response, reference = as.factor(Sriniv.mmu.predict_2C$cluster))

bm3 = get.best.model_C3(bncmark = bmr_3C)
test_task3 = makeClassifTask(data = Sriniv.mmu.predict_3C[1:22], target = "cluster")
predict3 = predict(bm3, task = test_task3, type = "prob")
CM3 = confusionMatrix(data = predict3$data$response, reference = as.factor(Sriniv.mmu.predict_3C$cluster))
