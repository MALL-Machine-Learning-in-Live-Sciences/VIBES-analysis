#Load
path = "~/git/BVMetaGenomics/data/SpeciesIntersect/"
Ravel_Sp = readRDS(paste0(path,"Ravel_Sps_Counts.rds"))
Sriniv_Sp = readRDS(paste0(path,"Sriniv_Sps_Counts.rds"))
...
#Specie
Ravel_Sp = select.target(dataset = Ravel_Sp, tget = "Nugent_score_category")
Ravel_Sp = remove.intermediate(dataset = Ravel_Sp)
Ravel_Sp = norm.dataset(data = Ravel_Sp)

#Sriniv
Sriniv_Sp = select.target(dataset = Sriniv_Sp, tget = "nugent")
Sriniv_Sp = remove.intermediate(dataset = Sriniv_Sp)
Sriniv_Sp = norm.dataset(data = Sriniv_Sp)
...
#FCBF
train.FCBF = FCBF.FS(data = Ravel_Sp, thold = 0.025)
#KW
train.KW = KW.FS(data = Ravel_Sp, thold = 90)
#LDM
targets = as.factor(Ravel_Sp$target)
cols = sapply(Ravel_Sp, is.numeric)
variables = Ravel_Sp[cols]
train.LDM = LDM.FS(data = Ravel_Sp,variables = variables,targets = targets, seed = 123)
#Raw
train.raw = Ravel_Sp
...
for (i in seq_along(ldata)) {
  saveRDS(ldata[[i]], file = paste0(path,"train/Ravel_Sp_",names(ldata[i]),"_",ncol(ldata[[i]])-1,".rds"))
}


# Benchmarks
set.seed(1312)
ML.exec = function(dataset, identi){
  require(mlr)
  require(methods)
  require(parallel)
  require(parallelMap)
  cores = detectCores()
  task = makeClassifTask(data = dataset, target = 'target',id = identi)
...
saveRDS(object = bmrs, paste0(path,"Benchmarks/BMR_Sp.rds"))
