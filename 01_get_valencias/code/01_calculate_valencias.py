...
cohort = re.sub(".csv", "", os.path.basename(input))
output = os.path.join(outpath, cohort)
plot = output
os.mkdir(output)
...
try:
    reference_centroids = pd.read_csv(reference,sep=',')
except:
...
#reading in table of samples to be tested against the centroids
    sample_data_OG = pd.read_csv(input,sep=',')
except:
...
sample_data_OG.to_csv("%s.csv" %(output),index=None)
...
if plot != None:
    ...
    #saving plot
    similarity_fig.savefig("%s.pdf" %(plot))
